...
public class Op3 : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider col)
    {
       ...
       {
            if (!correctAnswer && col.GetComponent<Cube>().correctNumber2Operator)
            {
                col.GetComponent<Cube>().hasBeenPlaced3 = true;
                audio.clip = yeahSound;
                audio.Play();
                correctAnswer = true;
                GetComponent<Renderer>().material.color = new Color(0, 0.9f, 0);
            }
            else if (!correctAnswer && col.GetComponent<Cube>().correctNumber2Operator == false)
            {
